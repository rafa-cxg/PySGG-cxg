...
class SGMeanRecall(SceneGraphEvaluation):
    ...
    def register_container(self, mode):
        ...
        #同样的再为2stage register
        self.result_dict[mode + "_2stage_mean_recall"] = {20: 0.0, 50: 0.0, 100: 0.0}
        self.result_dict[mode + "_2stage_mean_recall_collect"] = {
            20: [[] for i in range(self.num_cluster)],
            50: [[] for i in range(self.num_cluster)],
            100: [[] for i in range(self.num_cluster)],
        }
    ...
    def generate_print_string(self, mode):
        ...
        if cfg.SOLVER.VAL_2STAGE:
            result_str += "SGG eval: "
            for k, v in self.result_dict[mode + "_2stage_mean_recall"].items():
                result_str += " mR @ %d: %.4f; " % (k, float(v))
            result_str += " for mode=%s, type=2stage Mean Recall." % mode
            result_str += "\n"
            if self.print_detail:
                result_str += "Per-class 2stage recall@50: \n"
                for n, r in zip(
                        self.cluster_name_list, self.result_dict[mode + "_2stage_mean_recall_list"][50]
                ):
                    result_str += "({}:{:.4f}) ".format(str(n), r)
                result_str += "\n"
                result_str += "Per-class 2stage recall@100: \n"
                for n, r in zip(
                        self.cluster_name_list, self.result_dict[mode + "_2stage_mean_recall_list"][100]
                ):
                    result_str += "({}:{:.4f}) ".format(str(n), r)
                result_str += "\n\n"
        return result_str
    ...
    def calculate_mean_recall(self, mode):#collect里每个rel是每个图的结果列表，recall_list是每个图取平均的结果：
        ...
        '''计算2stage的mean recall'''
        for k, v in self.result_dict[mode + "_2stage_mean_recall"].items():
            sum_recall = 0
            num_rel_no_bg = self.num_cluster - 1
            for idx in range(num_rel_no_bg):#对于每个rel class,_mean_recall_collect[k][idx + 1]是个长度为照片总数的list
                if len(self.result_dict[mode + "_2stage_mean_recall_collect"][k][idx + 1]) == 0:
                    tmp_recall = 0.0
                else:
                    tmp_recall = np.mean(
                        self.result_dict[mode + "_2stage_mean_recall_collect"][k][idx + 1]
                    )
                self.result_dict[mode + "_2stage_mean_recall_list"][k].append(tmp_recall)
                sum_recall += tmp_recall

            self.result_dict[mode + "_2stage_mean_recall"][k] = sum_recall / float(num_rel_no_bg)
        return
...
class SGStagewiseRecall(SceneGraphEvaluation):
    def __init__(
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.RELATION_PROPOSAL_MODEL.PAIR_NUMS_AFTER_FILTERING < 0:
            self.eval_rel_pair_prop = cfg.MODEL.ROI_RELATION_HEAD.MAX_PROPOSAL_PAIR#4096
    ...
    def calculate_recall(
        ...
        '''pred_rels前两个元素是sub\obj的idx,最后一个是reltion label(background以外的rel取最大score)'''
        ...
        if self.instance_class_clustering:#default false
        ...
        pred_box_det_hit_idx = pred_box_loc_hit_idx[pred_det_hit_stat]#预测object lsbel正确的idx
        ...
        for idx in range(len(pred_box_det_hit_idx)):
            ...
            )#det_box_matching_results 的key和value和对应正确的pred idx和gt idx对应
        ...
        if self.rel_pn_on:
            ...
            for i in range(relness_score.shape[-1]):#shape[-1]=3,因为有3 iter,所以有3个sore

                roc_res = eval_roc(#res->result
        ...
        for topk_idx, topk in enumerate((20, 50, 100)):
            ...
            (
                ...
                rel_pair_mat,
                rel_init_pred_idx,
                pred_rel_loc_hit_idx,#todo rel_local到底是什么？
                gt_rel_loc_hit_idx,
                pred_rel_det_hit_idx,
                gt_rel_det_hit_idx,
            ...
            if topk == 100:
                pred_rel_scores = pred_boxlist.get_field("pred_rel_scores")#[4096,51]
                rel_scores, rel_class = pred_rel_scores[:, 1:].max(dim=1)
                det_score = pred_boxlist.get_field("pred_scores")#每个prop是否是object的score
                pairs = pred_boxlist.get_field("rel_pair_idxs").long()#[4096,2]

                rel_scores_condi_det = (#sub\obj prop score和rel score相称
                ...
                if not torch.isnan(rel_scores_condi_det).any():
                    ...
                    if not np.isnan(roc_res["auc"]):
                        self.result_dict[f"{mode}_{self.type}_pred_cls_auc-top{topk}"].append(#type：stage_recall
                            roc_res["auc"]
                        )

            # then we evaluate the full relationship triplets, sub obj detection and predicates
            rel_predicate_label = copy.deepcopy(selected_rel_pred[:, -1][rel_init_pred_idx])#selected_rel_pred:torch.Size([100, 3])
            rel_loc_pair_pred_label = copy.deepcopy(
                selected_rel_pred[:, -1][rel_loc_init_pred_idx]#todo init到底指的什么？
            )
            ...
            rel_hit_idx = (
                intersect_2d_torch_tensor(rel_pair_mat, to_cmp_gt_relationships)#是to_cmp的idx?
                .nonzero()
                .transpose(1, 0)
            )
            ...
            def stat_per_class_recall_hit(self, hit_type, gt_hit_idx):
                gt_rel_labels = gt_relations[:, -1]#gt_relations:[num_rel,3]
                ...
                per_cls_rel_hit[hit_rel_class_id, 0] += 1 #per_cls_rel_hit[51,2]
                per_cls_rel_hit[gt_rel_labels, 1] += 1
                self.relation_per_cls_hit_recall[hit_type][topk_idx] += per_cls_rel_hit#self.relation_per_cls_hit_recall是dict-4,每个元素3维（top20\50\100）
            ...
            # pre image relationship pairs hit counting#todo 难道idx有可能重复？
