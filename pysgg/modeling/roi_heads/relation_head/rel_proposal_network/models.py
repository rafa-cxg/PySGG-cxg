...
def gt_rel_proposal_matching(proposals, targets, fg_thres, require_overlap):
    ...
    for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
        ...
        # match any boxes
        locating_match = (ious > fg_thres).nonzero()  # [num_match, 2] 数量可能超过target数目
        locating_match_stat = torch.zeros((len(proposal)), device=device)
        locating_match_stat[locating_match[:, 1]] = 1#但凡有和gt对应上的proposal,该gt就设为1（说明该gt box有对应的prop）
        ...
        fg_boxpair_mat[:, locating_match_stat == 0] = 0#为了对称
        '''fg_pair_matrixs仅仅是前景prop组合组成的mat'''
        fg_pair_matrixs.append(fg_boxpair_mat)

        tgt_pair_idxs = torch.nonzero(tgt_rel_matrix != 0)#tgt_rel_matrix有可能是-1！！？

        assert tgt_pair_idxs.shape[1] == 2
        tgt_head_idxs = tgt_pair_idxs[:, 0].contiguous().view(-1)
        tgt_tail_idxs = tgt_pair_idxs[:, 1].contiguous().view(-1)
        tgt_rel_labs = tgt_rel_matrix[tgt_head_idxs, tgt_tail_idxs].contiguous().view(-1)#shape:num_gt_pair, labs 代表label， lab有可能是-1？？

        num_tgt_rels = tgt_rel_labs.shape[0]
        # generate binary prp mask
        num_prp = len(proposal)
        # num_tgt_rel, num_prp (matched prp head)
        binary_prp_head = is_match[tgt_head_idxs]#is_match有可能1对多，binary_prp_head：[num_head,num_prop]
        ...
        #binary_rel_mat对于predcls而言，就是对称且binary的tgt_rel_matrix，对于其他mode,受到prop数目影响
        fg_rel_triplets = []
        for i in range(num_tgt_rels):
            # generate binary prp mask 对于每一对gt rlation pair,可能对应的proposals（根据proposal和gt box的iou判断）。简单说，bi_match_head就是对应rel_pair的gt head所匹配的prop
            bi_match_head = torch.nonzero(binary_prp_head[i] > 0)#todo bi_match_head有可能不止一个元素，对于predcls怎么避免？
...
class RelAwareRelFeature(nn.Module):
    ...
    def forward(
        ...
        visual_feat_split = torch.split(visual_feat, [len(p) for p in rel_pair_inds], dim=0)#以image为单位
        ...
        for img_id, (proposal, vis_feats, pair_idx) in enumerate(
            ...
            #proposal_box_feat_extract：全连接降维
            ...
            if self.predictor_type == "hybrid":#default true
                relness_bin_logits = self.fusion_layer(relness_logits)#dim 50->1
        ...
        return (
            torch.cat(relness_logits_batch),#list of tensor size [num_pair,51]
            relness_matrix,
        )
