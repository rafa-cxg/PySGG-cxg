...
def loss_eval_hybrid_level(pre_cls_logits, rel_labels, loss):#loss分为两部分，第一部分是50rel分类，第二部分计算relateless,两个都是bce，所以2target可以
    ...
    bin_logits = selected_cls_logits[:, -1]#只取最后一个

    selected_labels = rel_labels[rel_labels != -1].long()

    onehot = torch.zeros_like(mulitlabel_logits)
    selected_fg_idx = squeeze_tensor(torch.nonzero(selected_labels > 0))#只是编号
    onehot[selected_fg_idx, selected_labels[selected_fg_idx] - 1] = 1#todo 和line 117矛盾？onehot[N,50]即不考虑no_rel，其他fg的one hot

    loss_val_mulabel = loss[0](inputs=mulitlabel_logits, targets=onehot, reduce=True)

    onehot = torch.zeros_like(bin_logits)
    onehot[selected_labels > 0] = 1#新的onehot,只考虑这个rel是不是有rel的onehot
    loss_val_bin = loss[1](inputs=bin_logits, targets=onehot, reduce=True)#loss_val_bin：scalar
...
class RelAwareLoss(nn.Module):
    ...
    def forward(self, pred_logit, rel_labels):
        if "focal" in self.pre_clser_loss_type:
            ...
            elif self.predictor_type == "hybrid":#ddefault
