...
def compute_on_dataset(model, data_loader, device, synchronize_gather=True, timer=None, logger=None):
    ...
    for _, batch in enumerate(tqdm(data_loader)):#这就是总数2500的地方
        ...
        with torch.no_grad():
            ...
            if timer:
                timer.tic()
            if cfg.TEST.BBOX_AUG.ENABLED:#default false
...
def inference(
        ...
        dataset_name,
        iou_types=("bbox",),
        # iou_types=(),#这样，就不会evalbbox相关的东西了
        box_only=False,
        device="cuda",
        expected_results=(),
    ...
    load_prediction_from_cache = cfg.TEST.ALLOW_LOAD_FROM_CACHE and output_folder is not None and os.path.exists(#FALSE
    ...
    total_timer.tic()
    if load_prediction_from_cache: #false
        ...
        predictions = compute_on_dataset(model, data_loader, device,
                                         synchronize_gather=cfg.TEST.RELATION.SYNC_GATHER,#gather the predictions during the training, rather than gathering all predictions after the training
